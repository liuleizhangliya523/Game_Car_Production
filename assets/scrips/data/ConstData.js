...
ConstData.CAR_NAME = [ //他人投注金额
    ...
    { key: "car12", name: "山菱" },
    { key: "save", name: "保护基金" },
    { key: "carMarket", name: "轿车" },
    { key: "suvMarket", name: "SUV" },
    { key: "manualCar", name: "手动型" },
    { key: "autoCar", name: "自动型" },
    { key: "manualSuv", name: "手动型" },
    { key: "autoSuv", name: "自动型" },
];
ConstData.getCarNameByKey = function (key) {
    let ele = ConstData.CAR_NAME.find(function (e) {
        return e.key === key;
    });
    if (ele != undefined) {
        return ele.name;
...
ConstData.getCarNameByIndex = function (idx) {
    let ele = ConstData.CAR_NAME.find(function (e, index) {
        return index === idx;
    });
    if (ele != undefined) {
        return ele.name;
